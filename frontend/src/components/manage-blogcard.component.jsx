...
export const ManagePublishedBlogPost = ({ blog, isPending }) => {
    ...
    return (
        ...
        <div className="flex gap-10 border-b mb-6 max-md:px-4 border-grey pb-6 items-center">
            ...
            <div className="flex flex-col justify-between py-2 w-full min-w-[300px]">
                <div>
                    <Link to={isPending ? (`/pending/${blog_id}`) : (`/blog/${blog_id}`)}><h1 className="blog-title mb-4 hover:underline">{title}</h1></Link>
                ...
                <div className="flex gap-6 mt-3">
                    ...
                    <button className="pr-4 py-2 underline text-red" onClick={(e) => deleteBlog(blog, access_token, e.target)}>Delete</button>
                    {isPending ? (<Link to={isPending ? (`/pending/${blog_id}`) : (`/blog/${blog_id}`)}><button className="pr-4 py-2 underline text-green">Approve</button></Link>) : null}
                </div>
            </div>
